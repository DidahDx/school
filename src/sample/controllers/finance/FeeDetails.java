...
public class FeeDetails implements Initializable {
   ...
   public void loadFinanceTable(){
      ...
      try {
         ResultSet rs= feeDao.loadTable(connection);
         fillTable(rs);
   ...
   public void search(ActionEvent actionEvent) {
      ...
      try{
         if(!(searchTextField.getText().isEmpty())){
            ...
            try{
               ResultSet rs=feeDao.search(Integer.parseInt(searchTextField.getText()),connection);
               fillTable(rs);
            }catch (SQLException e){
               e.printStackTrace();
            }
   ...
   public void setSelected(){
      ...
      try{
         ...
         ResultSet rs=studentDao.searchTable(fmt.getAdmissionNumber(),connection);
         while (rs.next()){
...
Connection connection=DBConnector.getConnection();
      ...
      try {
         if (form.getValue()=="All"){
            rs=feeDao.loadTable(connection);
            fillTable(rs);
         }else {
            rs=feeDao.searchForm(Integer.parseInt(form.getValue()),connection);
            fillTable(rs);
         }
   ...
   public void handleDelete(ActionEvent actionEvent) {
      ...
      try {
         ...
         if (answer.get()==ButtonType.OK){
            feeDao.deleteFees(finance_id,connection);
            loadFinanceTable();
         }
